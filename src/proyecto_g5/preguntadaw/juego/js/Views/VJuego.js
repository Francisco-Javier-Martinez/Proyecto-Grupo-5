export class VJuego {
    constructor() {
        ...
        // Actualizar el nombre del jugador desde localStorage 
        try {
            const nombreJugador = localStorage.getItem('jugadorNombre') || 'Jugador';
            const nombreElemento = document.querySelector('.nombre-jugador');
            if (nombreElemento) {
                nombreElemento.textContent = nombreJugador;
            }
        } catch (e) {
            console.warn('No se pudo obtener nombreJugador de localStorage:', e);
        }

        // Contador de preguntas
        if (localStorage.getItem('contadorPreguntas') !== null) {
            this.contadorPreguntas = parseInt(localStorage.getItem('contadorPreguntas'), 10);
        } else {
        ...
        if (puntosUrl !== null) {
            this.puntosJugador = Number(puntosUrl);
            localStorage.setItem('puntosJugador', this.puntosJugador.toString());
        }
        else if (localStorage.getItem('puntosJugador') !== null) {
            this.puntosJugador = Number(localStorage.getItem('puntosJugador'));
        }
        else {
    ...
    reiniciarJuego() {
        ...
        localStorage.removeItem('puntosJugador');
    ...
    cargarTimer() {
        ...
        const countdown = setInterval(() => {
            ...
            if (totalSeconds <= 0) {
                ...
                localStorage.setItem("puntosObtenidos", "0");
                localStorage.setItem("respuestaTexto", "Sin respuesta");
                window.location.href = `../juego/feeckback.html?tema=${this.tema}&correcta=false&puntos=${this.puntosJugador}`;
            }
        }, 1000);
    }

    mostrarPreguntas(arrayPreguntas) {
        localStorage.setItem('totalPreguntas', arrayPreguntas.length.toString());
        this.respuestaCorrecta = null;
        // Manda al ranking si no hay mas preguntas por responder
        if (this.contadorPreguntas >= arrayPreguntas.length) {
            //llevarme el puntaje final al ranking
            localStorage.setItem('puntosFinales', this.puntosJugador);
        ...
        if (arrayPreguntas.length > 0) {
            ...
            // Guardar información de la pregunta actual
            localStorage.setItem('respuestaExplicacion', pregunta.explicacion);
            localStorage.setItem('puntosPregunta', pregunta.puntuacion); // Guarda puntos de la pregunta

            // Crea elementos para la pregunta
            const titulo = `<h2>${pregunta.titulo || 'Sin título'}</h2>`;
            const imagen = pregunta.imagen && pregunta.imagen !== "" ?
                `<img src="../gestionadmin/php/views/img/preguntasimagenes/${pregunta.imagen}" alt="${pregunta.titulo}" class="imagen-pregunta">` :
    ...
    accionRespuestas(event) {
        ...
        // Obtener puntos de la pregunta actual desde localStorage
        const puntosPregunta = Number(localStorage.getItem('puntosPregunta'));
        const puntosObtenidos = esCorrecta ? Number(puntosPregunta) : 0;

        // Actualizar puntos totales
        if (esCorrecta) {
            this.puntosJugador += Number(puntosObtenidos);
        }
        localStorage.setItem('puntosJugador', this.puntosJugador.toString());
        // Guardar información de la respuesta actual
        localStorage.setItem('respuestaCorrecta', esCorrecta.toString());
        localStorage.setItem('respuestaTexto', boton.textContent);
        localStorage.setItem('respuestanLetra', boton.dataset.nLetra);
        localStorage.setItem('puntosObtenidos', puntosObtenidos.toString());
        ...
        setTimeout(() => {
            window.location.href = `../juego/feeckback.html?tema=${this.tema}&correcta=${this.respuestaCorrecta}&puntos=${this.puntosJugador}`;
        }, 1000);
    }
}